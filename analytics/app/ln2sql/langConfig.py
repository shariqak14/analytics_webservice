    ...
    def load(self, path):
        with open(self._generate_path(path)) as f:
            ...
            self.avg_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[0].replace(":", ",").split(","))),
                )
            )
            ...
            self.sum_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[1].replace(":", ",").split(","))),
                )
            )
            ...
            self.max_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[2].replace(":", ",").split(","))),
                )
            )
            ...
            self.min_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[3].replace(":", ",").split(","))),
                )
            )
            ...
            self.count_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[4].replace(":", ",").split(","))),
                )
            )
            ...
            self.junction_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[5].replace(":", ",").split(","))),
                )
            )
            self.junction_keywords = self.junction_keywords[
                1 : len(self.junction_keywords)
            ]
            self.junction_keywords = [
                keyword.lower() for keyword in self.junction_keywords
            ]

            self.disjunction_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[6].replace(":", ",").split(","))),
                )
            )
            self.disjunction_keywords = self.disjunction_keywords[
                1 : len(self.disjunction_keywords)
            ]
            self.disjunction_keywords = [
                keyword.lower() for keyword in self.disjunction_keywords
            ]

            self.greater_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[7].replace(":", ",").split(","))),
                )
            )
            self.greater_keywords = self.greater_keywords[
                1 : len(self.greater_keywords)
            ]
            self.greater_keywords = [
                keyword.lower() for keyword in self.greater_keywords
            ]

            self.less_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[8].replace(":", ",").split(","))),
                )
            )
            ...
            self.between_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[9].replace(":", ",").split(","))),
                )
            )
            self.between_keywords = self.between_keywords[
                1 : len(self.between_keywords)
            ]
            self.between_keywords = [
                keyword.lower() for keyword in self.between_keywords
            ]

            self.order_by_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[10].replace(":", ",").split(","))),
                )
            )
            self.order_by_keywords = self.order_by_keywords[
                1 : len(self.order_by_keywords)
            ]
            self.order_by_keywords = [
                keyword.lower() for keyword in self.order_by_keywords
            ]

            self.asc_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[11].replace(":", ",").split(","))),
                )
            )
            ...
            self.desc_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[12].replace(":", ",").split(","))),
                )
            )
            ...
            self.group_by_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[13].replace(":", ",").split(","))),
                )
            )
            self.group_by_keywords = self.group_by_keywords[
                1 : len(self.group_by_keywords)
            ]
            self.group_by_keywords = [
                keyword.lower() for keyword in self.group_by_keywords
            ]

            self.negation_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[14].replace(":", ",").split(","))),
                )
            )
            self.negation_keywords = self.negation_keywords[
                1 : len(self.negation_keywords)
            ]
            self.negation_keywords = [
                keyword.lower() for keyword in self.negation_keywords
            ]

            self.equal_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[15].replace(":", ",").split(","))),
                )
            )
            ...
            self.like_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[16].replace(":", ",").split(","))),
                )
            )
            ...
            self.distinct_keywords = list(
                map(
                    self.remove_accents,
                    list(map(str.strip, content[17].replace(":", ",").split(","))),
                )
            )
            self.distinct_keywords = self.distinct_keywords[
                1 : len(self.distinct_keywords)
            ]
            self.distinct_keywords = [
                keyword.lower() for keyword in self.distinct_keywords
            ]
