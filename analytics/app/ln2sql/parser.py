    ...
    def run(self):
        for table_of_from in self.tables_of_from:  # for each query
            ...
            if number_of_select_column == 0:
                ...
                for i in range(0, len(self.phrase)):
                    for column_name in self.columns_of_select:
                        if (self.phrase[i] == column_name) or (
                            self.phrase[i]
                            in self.database_object.get_column_with_this_name(
                                column_name
                            ).equivalences
                        ):
                ...
                column_of_select_index = (
                    -1
                )  # added by myself to choose the right column for aggregates
                for i in range(
                    0, len(select_phrases)
                ):  # for each select phrase (i.e. column processing)
                    select_type = []
                    ...
                    if i != len(select_phrases) - 1:
                        ...
                        for word in self.columns_of_select:
                            if word in phrase:
                                ...
                                for x in self.database_object.get_column_with_this_name(
                                    word
                                ).equivalences:
                                    if x in phrase:
                                        flag = 1
                        ...
                        if phrase and flag == 1:
                            ...
                            column = self.get_column_name_with_alias_table(
                                self.columns_of_select[column_of_select_index],
                                table_of_from,
                            )
                            self.select_object.add_column(
                                column, self.uniquify(select_type)
                            )
...
class FromParser(Thread):
    def __init__(
        self, tables_of_from, columns_of_select, columns_of_where, database_object
    ):
        Thread.__init__(self)
        self.queries = []
        self.tables_of_from = tables_of_from
    ...
    def is_direct_join_is_possible(self, table_src, table_trg):
        ...
        for column in fk_column_of_trg_table:
            if column.is_foreign()["foreign_table"] == table_src:
                return [
                    (table_src, column.is_foreign()["foreign_column"]),
...
class WhereParser(Thread):
    ...
    def predict_operator(self, current_column_offset, next_column_offset):
        ...
        if (
            len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1
        ) and (len(self.intersect(interval_offset, self.greater_keyword_offset)) >= 1):
            return "<"
        elif (
            len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1
        ) and (len(self.intersect(interval_offset, self.less_keyword_offset)) >= 1):
            return ">"
    ...
    def run(self):
            ...
            for i in range(0, len(phrase)):
                for table_name in self.database_dico:
                    ...
                    for column in columns:
                        if (phrase[i] == column.name) or (
                            phrase[i] in column.equivalences
                        ):
                ...
                for keyword in self.count_keywords:
                    if keyword in phrase_offset_string:  # before the column
                        if phrase_offset_string.find(keyword) + len(keyword) + 1 == len(
                            phrase_offset_string
                        ):
        ...
        for table_of_from in self.tables_of_from:
            ...
            for i in range(0, len(column_offset)):
                ...
                where_object.add_condition(
                    junction, Condition(column, operation_type, operator, value)
                )
                    ...
                    for table_name in self.database_dico:
                        ...
                        for column in columns:
                            if (phrase[i] == column.name) or (
                                ...
                                column_with_alias = self.get_column_name_with_alias_table(
                                    column.name, table_of_from
                                )
                    ...
                    for table_name in self.database_dico:
                        ...
                        for column in columns:
                            if (phrase[i] == column.name) or (
                                ...
                                order_by_object.add_column(
                                    column_with_alias, self.predict_order(phrase)
                                )
...
class Parser:
    ...
    def transformation_sort(cls, transition_list):
        ...
        return sorted(
            transition_list, key=functools.cmp_to_key(cls._myCmp), reverse=True
        )
    ...
    def parse_sentence(self, sentence, stopwordsFilter=None):
        ...
        for i in range(0, len(input_word_list)):
            for table_name in self.database_dico:
                if (input_word_list[i] == table_name) or (
                    input_word_list[i]
                    in self.database_object.get_table_by_name(table_name).equivalences
                ):
                ...
                for column in columns:
                    if (input_word_list[i] == column.name) or (
                        input_word_list[i] in column.equivalences
                    ):
                        ...
                        if (
                            (number_of_table_temp != 0)
                            and (number_of_where_column_temp == 0)
                            and (i == (len(input_word_list) - 1))
                        ):
        ...
        if irext:
            ...
            for idx, assigner in enumerate(assignment_list):
                if assigner in self.like_keywords:
                    ...
                    irext = irext.replace(
                        assigner, str(" " + maverickjoy_general_assigner + " ")
                    )
            ...
            for idx, x in enumerate(irext_list):
                ...
                if x == maverickjoy_like_assigner:
                    if (
                        index < len(irext_list)
                        and irext_list[index] != maverickjoy_like_assigner
                        and irext_list[index] != maverickjoy_general_assigner
                    ):
                        # added by myself ---------
                        if " ends with " in original_sentence:
                            ...
                            " starts with " in original_sentence
                            or " starts from " in original_sentence
                        ):
                            columns_of_values_of_where.append(
                                str(
                                    "'"
        ...
        for word in bigram_list:
            if (
                word in self.database_dico
                or word
                in self.database_object.get_table_by_name(table_name).equivalences
                or word in column_names
            ):
        ...
        if len(tables_of_from) > 0:
            ...
            for i in range(0, len(from_phrase)):
                for table in tables_of_from:
                    if (from_phrase[i] == table) or (
                        from_phrase[i]
                        in self.database_object.get_table_by_name(table).equivalences
                    ):
            ...
            if last_junction_word_index == -1:
                ...
                where_phrase = (
                    sum(from_phrases[last_junction_word_index + 1 :], []) + where_phrase
                )
        ...
        for word in from_phrase:
            for table in tables_of_from:
                if (word == table) or (
                    word in self.database_object.get_table_by_name(table).equivalences
                ):
