    ...
    def print_json(self, output):
        if len(self.columns) >= 1:
            if len(self.columns) == 1:
                ...
                output.write(
                    '\t\t"column": "'
                    + self.get_just_column_name(str(self.columns[0][0]))
                    + '",\n'
                )
                ...
                for i in range(0, len(self.columns)):
                    if i == (len(self.columns) - 1):
                        output.write(
                            '\t\t\t{ "column": "'
                            + self.get_just_column_name(str(self.columns[i][0]))
                            + '",\n'
                        )
                        output.write(
                            '\t\t\t  "type": "' + str(self.columns[i][1]) + '"\n'
                        )
                        output.write("\t\t\t}\n")
                    else:
                        output.write(
        ...
        if len(self.links) >= 1:
            ...
            for i in range(0, len(self.links)):
                string += (
                    ...
                    + "INNER JOIN "
                    + Color.END
                    + str(self.links[i][1][0])
                    + "\n"
                    + Color.BOLD
                    + "ON "
                    + Color.END
                    + str(self.links[i][0][0])
                    + "."
                    + str(self.links[i][0][1])
                    + " = "
                    + str(self.links[i][1][0])
                    + "."
                    + str(self.links[i][1][1])
                )
            return string
...
class Condition:
    ...
    def __str__(self):
        return (
            str(self.get_column_with_type_operation(self.column, self.column_type))
            + " "
            + str(self.get_pretty_operator(self.operator))
            + " "
            + str(self.value)
        )
        ...
        if len(self.columns) >= 1:
            if len(self.columns) == 1:
                ...
                for i in range(0, len(self.columns)):
                    if i == (len(self.columns) - 1):
                        output.write(
                            '\t\t\t{ "column": "' + str(self.columns[i][0]) + '",\n'
                        )
                        output.write(
                            '\t\t\t  "order": "' + str(self.columns[i][1]) + '"\n'
                        )
                        output.write("\t\t\t}\n")
                    else:
                        output.write(
